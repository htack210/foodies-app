...
export async function updateMealData(meal) {
  ...
  db.prepare(
    `UPDATE meals SET
      ...
      image = @image
    WHERE title = @org_title AND creator_email = @org_creator_email`
  ).run({
    ...meal,
    org_title: meal.org_title || meal.title, // fallback if not provided
    org_creator_email: meal.org_creator_email || meal.creator_email,
  });
}
