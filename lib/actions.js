...
import { updateMealData } from "./meals";
import { deleteMealData } from "./meals";
...
export async function updateMeal(prevState, formData) {
  ...
  const meal = {
    ...
    summary: formData.get("summary"),
    instructions: formData.get("instructions"),
    image: image && image.size > 0 ? image : formData.get("org_image"),
    creator: formData.get("creator"),
    creator_email: formData.get("creator_email"),
    slug: formData.get("slug"),
  };
  ...
  await updateMealData({ ...org, ...meal }); // Merge changes
  revalidatePath("/meals"); // This will refresh the meals list page
  revalidatePath(`/meals/${meal.slug}`); // refreshes the detail page for this meal
  return { message: "success" }; // Send 'success' when save is successful
}

export async function deleteMeal(prevState, formData) {
  const slug = formData.get("slug");
  await deleteMealData(slug);
